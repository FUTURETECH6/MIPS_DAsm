...
int mips_disassemble(mips_instruction_t *instruction, uint32_t number) {
    ...
    if (op == 0) {
        instruction->name = MIPS_REGISTER_INSTRUCTION_NAMES[funct_upper][funct_lower];
        instruction->type = MIPS_TYPE_R;
    } else if (op == 0x1c) {
        instruction->name = MIPS_REGISTER_C_INSTRUCTION_NAMES[funct_upper][funct_lower];
        instruction->type = MIPS_TYPE_R;
    } else if (op == 1) {
        instruction->name = MIPS_REGISTER_RT_INSTRUCTION_NAMES[rt_upper][rt_lower];
        instruction->type = MIPS_TYPE_I;
    } else {
        instruction->name = MIPS_ROOT_INSTRUCTION_NAMES[op_upper][op_lower];
        instruction->type = MIPS_TYPE_I;
    }
